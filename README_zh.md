<!--
  发布前请先把下面两个占位符全局替换掉：
    {{WEBSITE_URL}}    →  你的官方网站，例如 https://veo-automation.example.com
    {{EXTENSION_URL}}  →  你在 Chrome 应用商店的扩展页面链接（不要带查询参数）
  扩展链接已按各语言文件自动追加 ?hl={lang}。请对全部 7 个 README 文件
  （README.md、README_zh/ko/ja/de/fr/ru.md）做一次全局查找/替换。
-->

[![在 Chrome 应用商店下载](https://img.shields.io/badge/⬇_安装-Chrome_应用商店-success?style=for-the-badge&logo=googlechrome&logoColor=white)]({{EXTENSION_URL}}?hl=zh)
[![官方网站](https://img.shields.io/badge/🌐_官网-访问-blue?style=for-the-badge)]({{WEBSITE_URL}})
...
1. 打开 Chrome 应用商店里的 [**VEO Automation 扩展页面**]({{EXTENSION_URL}}?hl=zh)。
...
在侧边栏里登录即可查看你当前的额度。在 [{{WEBSITE_URL}}]({{WEBSITE_URL}}) 管理你的套餐。

---

## 🌐 链接

- **安装：** [Chrome 应用商店]({{EXTENSION_URL}}?hl=zh)
- **官网：** [{{WEBSITE_URL}}]({{WEBSITE_URL}})
